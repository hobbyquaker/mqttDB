    ...
    query(id, payload, force) {
        ...
        if (payload === '') {
            ...
            try {
                ...
                this.views[id].serror = 'script creation: ' + err.message;
            ...
            this.execView(id, (err, res) => {
                if (err) {
                    this.views[id].error = err;
                    delete this.views[id].res;
                    this.emit('view', id, {error: err, _id: id});
                } else {
                    delete this.views[id].error;
                    if (this.views[id].serror) {
                        this.views[id].error = this.views[id].serror;
                    }
    ...
    execView(id, callback) {
        ...
        viewDomain.run(function ()  {
            setTimeout(function () {
                ...
                console.log('!!!', context._meta.res);
                callback(null, oe.clone(context._meta.res.sort()));
            }, 0);
        });
    }
